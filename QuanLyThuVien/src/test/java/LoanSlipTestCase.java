...
// Do trên giao diện đã fix cứng những field search nên em tự cho dữ liệu phù hợp 
    ...
    public void testCreateLoanSlip() {
        ...
        BookModel b1 = listBook.get(0);
        b1.setQuantity(1);
        BookModel b2 = listBook.get(1);
        b2.setQuantity(1);
        orderBook.add(b1);
        orderBook.add(b2);
        Integer rs1 = loanSlipService.creatLoanSlip(orderBook, 1, "2" , 0);
        // 1 --> Thành công 
    ...
    public void testUpdateStatusBook() {
        ...
        Assertions.assertEquals(rs2, -1);
        // 0 --> thành công
        LoanSlipModel loanSlip3 = loanSlipService.findByBookIDAndReaderID(10, 8).get(1);
        Integer rs3 = loanSlipService.updateBook(loanSlip3);
        Assertions.assertEquals(rs3, 0);
//      // >0 --> thành công + phạt tiền
        LoanSlipModel loanSlip4 = loanSlipService.findByBookIDAndReaderID(5, 10).get(0);
